...
export default function Inmates(props) {
  ...
  const Action = (data) => {
    ...
    const remove = (id) => {
      axios
        .delete(`${config.baseUrl}discussion/comment/${id}/${id}`, {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
        })
    ...
    return (
      <div className="d-flex align-items-center">
        <Button
          ...
          onClick={() => {
            history.push({
              pathname: `/discussion-info-comments/${data._id}`,
              state: {},
            });
          }}
        >
          <i className="fa fa-eye pr-2"></i> View Comment(s)
        </Button>
        <Button
